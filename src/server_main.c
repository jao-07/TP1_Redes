...
int main(int argc, char *argv[]){
    ...
    battle_message->client_hp = 100;
    battle_message->server_hp = 100;
    ...
    while(1){
        ...
        if(battle_message->type == MSG_ESCAPE || !battle_message->client_hp || !battle_message->server_hp){
            ...
            snprintf(battle_message->message, sizeof(battle_message->message), "- Turnos jogados: %d\n", turns);

            if(battle_message->client_hp && battle_message->server_hp) snprintf(battle_message->message + strlen(battle_message->message), MSG_SIZE - strlen(battle_message->message), "Obrigado por jogar!\n");
            if(battle_message->client_hp && !battle_message->server_hp) snprintf(battle_message->message + strlen(battle_message->message), MSG_SIZE - strlen(battle_message->message), "Você derrotou a frota inimiga!\n");
            if(!battle_message->client_hp && battle_message->server_hp) snprintf(battle_message->message + strlen(battle_message->message), MSG_SIZE - strlen(battle_message->message), "Sua nave foi destruída!\n");
            if(!battle_message->client_hp && !battle_message->server_hp) snprintf(battle_message->message + strlen(battle_message->message), MSG_SIZE - strlen(battle_message->message), "Ambas as naves foram destruídas!\n");
